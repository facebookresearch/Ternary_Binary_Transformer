...
#
# 2023.02.01 - Add support for two-set quantization and using KD loss for logit distillation
...
def parse_args():
    ...
    args.output_dir = f'./output_{args.dataset_name}/{args.weight_bits}_{args.input_bits}_{args.distill_encoder}_{args.distill_decoder}_{args.num_train_epochs}_{args.learning_rate}_quant'
    if args.new_distill_map:
        args.output_dir += '_new'
    if (not args.pred_distill) and (not args.hid_distill) and (not args.attn_distill):
...
def main():
    ...
    if args.model_name_or_path:
        ...
        student_config = QBartConfig.from_pretrained(args.teacher_model,
                                                     ...
                                                     sym_quant_ffn_attn=args.sym_quant_ffn_attn,
                                                     sym_quant_qkvo=args.sym_quant_qkvo)
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if args.pred_distill:
                    ...
                    logits_shape = student_outputs.logits.shape
                    logits_loss = criterion_kd(student_outputs.logits.reshape(-1, logits_shape[-1]), teacher_outputs.logits.reshape(-1, logits_shape[-1]))

                if args.hid_distill or args.attn_distill:
